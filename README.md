...
Authentication uses a Nimriz **Workspace API key**. Create one in your Nimriz dashboard under **Settings → Integrations → API access**. The key is tied to a single workspace, so you do not need to enter a workspace ID separately.

Add a **Nimriz API** credential in n8n and paste the key. Use the **Test** button to confirm it is valid — the credential is checked against `GET /api/v1/whoami`.
